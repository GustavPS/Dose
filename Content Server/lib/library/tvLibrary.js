                    ...
                    if (!alreadyAdded) {
                        this.metadata.getShowMetadata(serieName).then(result => {
                            ...
                            this.metadata.insertShowMetadata(metadata, images, trailer, internalSerieID).then(() => {
                                ...
                                sockets.emit("newShow", {"id": internalSerieID, "title": metadata.name, "overview": metadata.overview, "backdrop": back} )
                ...
                if (!alreadyAdded) {
                    this.metadata.getEpisodeMetadata(serieTmdbId, seasonNumber, episodeNumber).then(result => {
                        ...
                        this.metadata.insertEpisodeMetadata(metadata, internalSerieID, seasonNumber, episodeNumber).then(() => {
                            ...
                            sockets.emit("newEpisode", {"serie_id": internalSerieID, "internalepisodeid": internal_episode_id, "season": seasonNumber, "episode": episodeNumber, "season_poster": poster.poster_path} )
                      });
                            resolve(true);
                        });
                    }).catch(async (error) => {
