...
...(_webpack.isWebpack5?{emitOnErrors:!dev}:{noEmitOnErrors:dev}),checkWasmTypes:false,nodeEnv:false,splitChunks:isServer?false:splitChunksConfig,runtimeChunk:isServer?_webpack.isWebpack5&&!isLikeServerless?{name:`${dev?'':'chunks/'}webpack-runtime`}:undefined:{name:_constants2.CLIENT_STATIC_FILES_RUNTIME_WEBPACK},minimize:!(dev||isServer),minimizer:[// Minify JavaScript
...
version:`${"10.0.8"}|${nextPublicVariables}|${nextEnvVariables}|${configVars}`,cacheDirectory:_path.default.join(dir,'.next','cache','webpack')};// Adds `next.config.js` as a buildDependency when custom webpack config is provided
